...
export function AppSidebar({
  ...
  const dotColor = (s?: string) =>
    s === 'operational' ? 'bg-emerald-500' : s === 'degraded' ? 'bg-yellow-400' : 'bg-zinc-700';

  const isOperational = status?.overall === 'operational';

  return (
    <aside className="w-[220px] shrink-0 h-screen flex flex-col bg-[#09090B] overflow-hidden">
      {/* Logo */}
      <div className="px-5 pt-6 pb-5">
        <span className="text-[17px] font-bold tracking-[-0.04em] text-white leading-none block">
          Prozessia.
        </span>
        <span className="text-[8px] font-semibold tracking-[0.2em] text-zinc-500 uppercase mt-[4px] block">
          AI Agency
        </span>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 pb-3 flex flex-col gap-4">
        {navGroups.map((group) => (
          <div key={group.label}>
            <p className="text-[9.5px] font-semibold tracking-[0.12em] text-zinc-600 uppercase px-2 mb-1">
              {group.label}
            </p>
            <nav className="flex flex-col gap-0.5">
              {group.items.map((item) => (
                ...
                >
                  <span className={activeView === item.id ? 'text-white' : 'text-zinc-500'}>
                    {item.icon}
                  </span>
                  {item.label}
                </button>
      ...
      {/* Demo-Modus Hinweis */}
      <div className="mx-3 mb-3 rounded-[8px] border border-white/[0.07] bg-white/[0.03] px-3 py-2.5">
        <p className="text-[11px] font-semibold text-zinc-300 mb-0.5">Demo-Modus</p>
        ...
        <a
          ...
          className="inline-block mt-2 text-[10.5px] font-semibold text-zinc-300 hover:text-white transition-colors"
        >
          Demo buchen →
        </a>
      </div>
